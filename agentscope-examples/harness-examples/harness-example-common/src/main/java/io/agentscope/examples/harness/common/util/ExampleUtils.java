 ...
 */
package io.agentscope.examples.harness.common.util;
...
public class ExampleUtils {
    ...
    public static Msg userMsg(String text) {
        return MsgUtils.textMsg(null, MsgRole.USER, text);
    }

    public static RuntimeContext ctx(String sessionId, String userId) {
        return RuntimeContext.builder().sessionId(sessionId).userId(userId).build();
    }

    public static RuntimeContext ctx(String sessionId) {
        return RuntimeContext.builder().sessionId(sessionId).build();
    }

    public static void startChat(HarnessAgent agent, RuntimeContext ctx) throws IOException {
        startHarnessChat(agent, ctx);
    }

    public static void startHarnessChat(HarnessAgent agent, RuntimeContext ctx) throws IOException {
        System.out.println("=== Chat Started ===");
        System.out.println("Type 'exit', 'quit', or 'q' to quit\n");
        ...
        while (true) {
            ...
            try {
                runHarnessTurn(agent, ctx, question);
                System.out.println();
            } catch (Exception e) {
                System.err.println("\nError: " + e.getMessage());
                e.printStackTrace();
    ...
    public static void runHarnessTurn(HarnessAgent agent, RuntimeContext ctx, String question) {
        Msg userMsg = userMsg(question);
        System.out.print("Agent> ");
        try {
            streamHarnessTurn(agent, ctx, userMsg);
        } catch (Exception e) {
            if (e instanceof UnsupportedOperationException) {
                System.err.println(
    ...
    private static void streamHarnessTurn(HarnessAgent agent, RuntimeContext ctx, Msg userMsg) {
        AtomicBoolean hasPrintedThinkingHeader = new AtomicBoolean(false);
        AtomicBoolean hasPrintedTextHeader = new AtomicBoolean(false);
        AtomicBoolean hasPrintedTextSeparator = new AtomicBoolean(false);
        AtomicReference<String> lastThinkingContent = new AtomicReference<>("");
    ...
    private static void printResponseContent(Msg response) {
        if (response == null) {
            System.out.print("[No response]");
            return;
        }
