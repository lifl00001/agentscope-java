...
/**
 * HTTP entry point for the sandbox Data Agent.
 *
 * <pre>
 * GET  /        — usage hint
...
public class DataAgentController {
    ...
    @GetMapping("/")
    public String index() {
        return "Sandbox Data Agent — POST /query with"
                + " {\"sessionId\":\"...\",\"userId\":\"...\",\"question\":\"...\"}";
    }

    /**
     * Processes a natural-language question about the Chinook database.
     *
     * <p>The agent sandbox and memory are scoped by {@code userId}: the same user always resumes
     * the same sandbox (and sees the same {@code MEMORY.md}) regardless of {@code sessionId}. The
     * conversation context (messages) is scoped by {@code sessionId}.
     */
    @PostMapping("/query")
