...
Same module as simple routing (`agentscope-examples/multiagent-patterns/routing`). Graph variant lives under `src/main/java/.../routing/graph/`:

- **RoutingGraphConfig.java** – StateGraph, agents, routingGraph bean
- **RoutingGraphService.java** – invokes graph
- **RoutingGraphRunner.java** – optional demo (`routing-graph.runner.enabled=true`)
- **node/PreprocessNode.java**, **PostprocessNode.java**
- Shared **tools/** (GitHub, Notion, Slack) under `routing/simple/tools` or `routing/graph/tools`
...
(Artifact is `routing`; graph and simple are in the same module.)
